...
## F. RAMEN 레시피 실험 (다른 라인, 200k 스텝)

Team-RAMEN 이 공개한 IROS2026 서브태스크 정책을 뜯어본 뒤 그 레시피를 우리 데이터에
적용한 것 (`RAMEN_COMPARISON.md`). 우리 라인과 **batch 16 / 200k 스텝 / REAL_G1 태그**가
다르다. 데이터는 `G1_Dex1_IKEA_leg_30hz`(insert 2 배, 3 태스크).

| 런 | ckpt | 한 줄 설명 |
|---|---:|---|
| `..._ikea_ramen_b16_leg_r` | 18 개 (64k~200k) | armvel 60 차원을 **`REAL_G1` 태그로 등록** — projector 가 미사용 slot 10 대신 사전학습된 **slot 25** 에서 출발한다 |
| `..._ikea_ramen_b16_leg_reef` | 18 개 (56k~192k) | 위와 **EEF 하나만 다름** — 손목 EEF 9D 를 팔 관절과 **함께 예측** (action 16 → 34). RAMEN 레시피의 완전판 |

**⚠️ 둘 다 val 미실시.** 베스트 체크포인트를 고르지 않아 18 개를 전부 옮겼다. 스캔하면
각각 12 GB 로 줄어든다. 스캔할 때는 `scan_ikea.py --embodiment-tag REAL_G1` 이 필요하다 —
태그가 안 맞으면 `MODALITY_CONFIGS` 에 항목이 없어 체크포인트 하나 열기 전에 KeyError 로 죽는다.

> slot 25 가 "맞는 사전지식"이라는 보장은 없다. 그 가중치는 NVIDIA 의 49 차원 컬럼 의미에
> 맞춰진 것이고 우리 60 차원은 legs 가 [0:12] 로 시작한다. 컬럼 대응이 어긋나므로
> **"실제 로봇 데이터를 본 적 있는, 잘 조건화된 행렬"** 이상은 아니다. 중립이거나 해로울
> 수도 있다 — 그래서 측정 대상이다.
...
| 위치 | 크기 | 설명 |
|---|---:|---|
| `pi05/pi05_ikea_expert` | 20G | π0.5 expert-only. ckpt 022500 / 030000 + `val_scores.json` |

> exFAT 은 심볼릭 링크를 못 만든다. 원본의 `checkpoints/last -> 030000` 은 `last.txt` 로
> 대체했다 — 데이터가 아니라 포인터라 손실은 없다.

## 이 전송에 없는 것

| | 크기 | 이유 |
|---|---:|---|
| `pi05_ikea_full` | 93G | **학습 중** (22,500/30,000) |
| `models--lerobot--pi05_base` (HF 캐시) | 14G | π0.5 베이스 — 재다운로드 가능 |

`sync_models_to_t9.sh` 를 다시 돌리면 학습이 끝난 것이 추가된다
